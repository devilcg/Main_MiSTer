...
static void get_display_name(direntext_t *dext, const char *ext, int options)
{
	...
	if (!names_loaded)
	{
		...
		{ FILE *_f=fopen("/tmp/nd.txt","a"); if(_f){fprintf(_f,"path='%s' try='%s'\n",scanned_path,names_path);fclose(_f);} }
		int size = FileLoad(names_path, 0, 0);
		{ FILE *_f=fopen("/tmp/nd.txt","a"); if(_f){fprintf(_f,"size=%d\n",size);fclose(_f);} }
		if (size)
		{
	...
	if (names)
	{
		...
		if (transl)
		{
			...
			dext->altname[len] = 0;
			{ FILE *_f=fopen("/tmp/nd.txt","a"); if(_f){fprintf(_f,"OK: '%s'\n",dext->altname);fclose(_f);} }
			return; // 번역 완료
		}
